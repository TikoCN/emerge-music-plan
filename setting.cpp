...
void Setting::writeData()
{
    ...
    if(dataFile.open(QIODevice::Text | QIODevice::WriteOnly)){
        ...
        for(int i=0; i<player->tableList.size(); i++){
            ...
            table.insert("musicNumber", player->tableList[i]->musics.size());
            //插入列表的信息
            if(!player->tableList[i]->isDir){
                QStringList coreKey;
                for(int j=0; j<player->tableList[i]->musics.size(); j++){
                    coreKey.append(QString::number(player->tableList[i]->musics[j]->coreId));
                }
        ...
        tableJson.insert("size", player->tableList.size());
