...
void TLog::logDebug(const QString &str) {
    emit doLog(DEBUG, str, QThread::currentThread()->objectName(),
...
void TLog::logUser(const QString &str) {
    emit doLog(USER_DO, str, QThread::currentThread()->objectName(),
               reinterpret_cast<qint64>(QThread::currentThreadId()));
}

void TLog::logError(const QString &str) {
    emit doLog(ERROR, str, QThread::currentThread()->objectName(),
               reinterpret_cast<qint64>(QThread::currentThreadId()));
}

void TLog::logRelease(const QString &str) {
    emit doLog(RELEASE, str, QThread::currentThread()->objectName(),
               reinterpret_cast<qint64>(QThread::currentThreadId()));
}
...
void TLog::logIgnore(const QString &str) {
    emit doLog(IGNORE, str, QThread::currentThread()->objectName(),
               reinterpret_cast<qint64>(QThread::currentThreadId()));
}